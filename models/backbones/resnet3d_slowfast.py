...
class ResNet3dSlowFast(BaseModule):
    ...
    def __init__(self,
        ...
        # 创建TAU
        self.TemporalAttention = TemporalAttention(d_model=96)
    ...
    def forward(self, x: torch.Tensor) -> tuple:
        ...
        N, C, T, H, W = x.shape  # 2, 3, 32, 256, 256
        x_TAU = x
        x_TAU = x_TAU.reshape(N, C * T, H, W)  # [2, 96, 256, 256]
        x_TAU = self.TemporalAttention(x_TAU)  # [2, 96, 256, 256]
        x_TAU = x_TAU.reshape(N, C, T, H, W)  # [2, 96, 256, 256]
        ...
        N, C, T, H, W = x_slow.shape
        x_slow_TAU = x_slow
        x_slow_TAU = x_slow_TAU.reshape(N, C * T, H, W)
        x_slow_TAU = self.TemporalAttention(x_slow_TAU)
        x_slow_TAU = x_slow_TAU.reshape(N, C, T, H, W)

        N, C, T, H, W = x_fast.shape
        x_fast_TAU = x_fast
        x_fast_TAU = x_fast_TAU.reshape(N, C * T, H, W)
        x_fast_TAU = self.TemporalAttention(x_fast_TAU)
        x_fast_TAU = x_fast_TAU.reshape(N, C, T, H, W)
